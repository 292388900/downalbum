...
//#define BUFFER_SIZE 4096*4096   //必须是64的倍数
static char * Buffer=NULL;
//MD5文件摘要
MD5VAL md5File(FILE * fpin)
{
	const int BUFFER_SIZE=4096*4096;   //必须是64的倍数
...
//MD5文件摘要
MD5VAL md5File(CString strFileName)
{
	/************************************************************************/
	MD5VAL val={0x67452301,0xefcdab89,0x98badcfe,0x10325476};
	unsigned int &a=val.a, &b=val.b, &c=val.c, &d=val.d;
	...
	if (hFile == INVALID_HANDLE_VALUE){
		TRACE("CreateFile失败,错误代码:%d\r\n", GetLastError());
		return val;
	}
	// 创建文件映射对象
	HANDLE hFileMap = CreateFileMapping(hFile, NULL, PAGE_READONLY, 0, 0, NULL);
	if (hFileMap == NULL){
		TRACE("CreateFileMapping失败,错误代码:%d\r\n", GetLastError());
		return val;
	}
	...
	BUFFER_SIZE=1000*dwGran;
	Buffer=new char[BUFFER_SIZE+64];
	char * buf=Buffer;
	i=0;
	// 块大小
	DWORD dwBlockBytes = BUFFER_SIZE;
	if (qwFileSize < BUFFER_SIZE)
	...
	while (qwFileSize > 0){
		...
		i+=count;
		if(count==BUFFER_SIZE){
			buf=(char *)lpbMapAddress;
			co=BUFFER_SIZE;
		}else{
			buf=Buffer;
			memcpy(buf,lpbMapAddress,dwBlockBytes);
