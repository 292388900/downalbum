...
#include "StarLib/Process/Process.h"

extern "C" {
#include <StarLib/LicenseLib/libcpuid/libcpuid.h>
}

// 串行化宏实现
IMPLEMENT_SERIAL(CSerialManager,CObject,0);

void CSerialManager::Serialize(CArchive& ar)
{
	if (ar.IsStoring()){
		//写
		ar << m_strBoring1;
...
CSerialManager::CSerialManager(LPCTSTR lpszPrefix)
{
	Star::Common::InitializeSeed();
	m_strPrevfix=lpszPrefix;
	CString strText;
	int nLen=Star::Common::RandIntNum(2048,4096);
	char*pBuff=new char[nLen+1];
	for ( int i=0; i<nLen; ++i ){
		pBuff[i]=Star::Common::RandIntNum(32,126);
	}
	pBuff[nLen]=0;
	strText=pBuff;
	delete pBuff;
	...
	int nDelta=nLen/8;
	m_strBoring1=strText.Mid(0,nDelta);
	m_strBoring2=strText.Mid(nDelta,nDelta*2);

	m_strSerial=strText.Mid(nDelta*2,nDelta*3);

	m_strBoring3=strText.Mid(nDelta*3,nDelta*4);
	m_strBoring4=strText.Mid(nDelta*4,nDelta*5);
	m_strBoring5=strText.Mid(nDelta*5,nDelta*6);

	m_strMachineCode=strText.Mid(nDelta*6,nDelta*7);

	m_strBoring6=strText.Mid(nDelta*7);
	MakeMachineCode();

	//没办法啊 防破解加入一个父进程检测
#if 0
	VMPBEGIN;
	DWORD dwPPID=Star::Process::GetParentProcessID(GetCurrentProcessId());
	DWORD dwPPID2=Star::Process::GetParentProcessID2(GetCurrentProcessId());
	if ( dwPPID==0 || dwPPID2==0 || dwPPID!=dwPPID2 ){
		m_strSerial=m_strSerial.Left(20);
	}else{
		CString strName=Star::Process::GetProcessNameByPID(dwPPID);
		if ( StrStrI(strName,"explorer.exe")==NULL && StrStrI(strName,"cmd.exe")==NULL ){
			m_strSerial=m_strSerial.Left(20);
		}else{
...
// 	}
// 	reg.MakeUpper();
// 	m_strMachineCode = m_strPrevfix + reg;
...
mc.vendor = (char)data.vendor;
mc.family = (char)data.family;
mc.model = (char)data.model;
mc.stepping = (char)data.stepping;
}

//转化为A-Z之间的字母
BYTE*p=(BYTE*)&mc;
for ( int i=0; i<sizeof(mc); ++i,++p ){
*p=*p%26+'A';
}

char hardcode[MAX_PATH]={};
memcpy(hardcode,&mc,sizeof(mc));
m_strMachineCode=hardcode;
m_strMachineCode = m_strPrevfix + m_strMachineCode;
return m_strMachineCode;
...
//CPUid在重装系统后确实不变，但是第二组获取的cpuid全是0。很多主板可以硬屏蔽对CPU序列号读取命令
//这里就用MAC地址了
CString& CSerialManager::MakeMachineCode()
{
	CString strmachine;

	AfxBeginThread(ThreadGetMac,this);
	strmachine=Star::Common::GetMacByAdapter();
	//strmachine=Star::Common::GetCpuId();

	CString code[16]={_T("ad"),_T("qu"),_T("eh"),_T("np"),_T("zc"),_T("ru"),_T("vy"),_T("gk"),
		_T("pt"),_T("fj"),_T("xb"),_T("ox"),_T("wa"),_T("ei"),_T("nr"),_T("im")};
	CString reg,stred;
	int num;

	stred = strmachine ;
	stred.MakeLower();
	int nLen=stred.GetLength()<10?stred.GetLength():10;
	for(int i=0;i<nLen;i++){
		char p=stred.GetAt(i);
		if(p>='a'&&p<='f')
			num=p-'a'+10;
...
UINT CSerialManager::ThreadGetMac(LPVOID lpParam)
{
	CString strmachine;

	strmachine=Star::Common::GetMacByCmd();
	return 0;
}
