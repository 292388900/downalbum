...
bool Cexif::DecodeExif()
{
    ...
    if (m_pbFile[0] != 0xff || m_pbFile[1] != M_SOI){
        return 0;
    }
    ...
    for(;;){
        ...
        lh = m_pbFile[nFilePointer];
        ll = m_pbFile[nFilePointer+1];
		...
		Data = m_pbFile + nFilePointer;
        Sections[SectionsRead].Size = itemlen;
        Sections[SectionsRead].Data = Data;
		nFilePointer += itemlen;
                           ...
                           EXIFINFO * const m_exifinfo, unsigned char ** const LastExifRefdP )
{
    ...
    for (de=0;de<NumDirEntries;de++){
        ...
        switch(Tag){
            ...
            case TAG_MAKE:
				m_pCameraMake = (PCHAR)ValuePtr;
                strncpy(m_exifinfo->CameraMake, (char*)ValuePtr, 31);
                break;

            case TAG_MODEL:
				m_pCameraModel = (PCHAR)ValuePtr;
            ...
            case TAG_MAXAPERTURE:
                ...
                */
                if (m_exifinfo->ApertureFNumber == 0){
                    m_exifinfo->ApertureFNumber = (float)exp(ConvertAnyFormat(ValuePtr, Format)*log((double)2)*0.5);
                }
                break;
                ...
                */
                if (m_exifinfo->ExposureTime == 0){
                    m_exifinfo->ExposureTime = (float)
                        (1/exp(ConvertAnyFormat(ValuePtr, Format)*log((double)2)));
                }
                break;
...
bool Cexif::UpdateOriginalDateTime(const char *lpszNewTime)
{
	if ( m_pbFile!=NULL && m_pDateTime!=NULL ){
		strcpy_s(m_pDateTime,20,lpszNewTime);
	}
	return true;
}

bool Cexif::OpenFile(LPCTSTR lpszFileName)
{
	m_hFile = CreateFile(lpszFileName, GENERIC_READ|GENERIC_WRITE, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	...
	m_pbFile = (PBYTE)MapViewOfFile(m_hFileMapping, FILE_MAP_READ|FILE_MAP_WRITE, 0, 0, 0);
	if ( m_pbFile==NULL ){
