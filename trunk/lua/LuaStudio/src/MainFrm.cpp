...
CMainFrame::CMainFrame()
{
	...
	last_page_ = 0;	// ostatnio wywo砤na strona (zak砤dka) w pude砶u opcji
...
int CMainFrame::OnCreate(LPCREATESTRUCT create_struct)
{
	...
	{		// dodanie pola do wy渨ietlania aktualnego wiersza i kolumny
		...
		status_bar_wnd_.SetPaneInfo(2, id, style, 16);	// szeroko滄 obrazka
	...
	case WM_PAINT:
		{
			...
			if (ret == 0)
			{
				...
				int borders[3];
				(*pfn_old_proc_)(wnd,SB_GETBORDERS,0,(LPARAM)borders);		// grubo滄 obw骴ki
...
afx_msg LRESULT CMainFrame::OnStartDebugger(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
	...
	status_bar_wnd_.InvalidateRect(&rect);		// obrazek pch硑 do przerysowania
	return 0;
}
...
void CMainFrame::OnClose()
{
	...
	{						   // zapisanie po硂縠nia okna g丑wnego
...
void CMainFrame::OnAssemble()
{
	...
	//if (theApp.global_.IsDebugger())	// dzia砤 debugger?
	//{
	//	if (AfxMessageBox(IDS_STOP_DEBUG,MB_OKCANCEL) != IDOK)
	//		return;
	//	ExitDebugMode();			// wyj渃ie z trybu debuggera
	//}
	...
	if (CDocument* doc= view->GetDocument())
	{
		...
		//if (doc->IsModified())
		//{
		//	//lua脚本被修改,断点所在的行可能已经偏移,方便起见清除先前所有断点
		...
		if (!path.IsEmpty())
		{
			...
			if (lua->LoadFile(path, msg))
			{
				...
				//view->ClearAllBreakpoints();	//不再清除断点
			}
			else
			{
				if (int line= ParseLine(path, msg))
...
void CMainFrame::OnSimBreakpoint() 
{
	...
	{
		int line= view->GetCurrLineNo();	// bie抗cy wiersz
		...
		/*
		// ustawienie miejsca przerwania w kodzie wynikowym odpowiadaj筩ym bie抗cemu wierszowi
...
void CMainFrame::OnSimBreak()		// stop execution
{
	...
	GetDebugger().Break();		// przerwanie dzia砤j筩ego programu
	DelayedUpdateAll();
...
void CMainFrame::OnSimGoToLine()
{
	...
	//	return;
	//}
	//else if (flg & Defs::DBG_DATA)			// wiersz z danymi zamiast rozkaz體?
	//{
...
{
	//cmd_ui->Enable( theApp.global_.IsDebugger() &&	// jest dzia砤j筩y debugger
	//  !theApp.global_.IsProgramRunning() &&		// oraz zatrzymany
	//  !theApp.global_.IsProgramFinished() &&		// niezako馽zony program
...
void CMainFrame::OnSimStepInto()	// wykonanie bie抗cej instrukcji
{
	if (!GetDebugger().IsStopped())
		return;
...
{
	//if (!theApp.global_.IsDebugger())	// nie ma dzia砤j筩ego debuggera?
	//  return;
...
{
	cmd_ui->Enable(GetDebugger().IsActive());
	//  cmd_ui->Enable( true );	// jest dzia砤j筩y debugger?
...
//    if (IO_window_.IsWaiting())
//      IO_window_.ExitModalLoop();
//    theApp.global_.GetDebugger().AbortProg();	// przerwanie dzia砤j筩ego programu
//  }
//  OnSimDebugStop();
//}
...
{
	last_page_ = Options(5);		// opcje wygl筪u okna pami阠i
}
