#pragma once
#include "afx.h"
#include <StarLib/LicenseLib/AntiDebug.h>
...
{
	// 串行化宏定义
	DECLARE_SERIAL(CSerialManager)
public:
	// 串行化函数 Serialize() 
	virtual void Serialize(CArchive& ar);

public:
	CString m_strBoring1;
	CString m_strBoring2;
	DWORD m_dwBoring1[64];

	CString m_strSerial;

	CString m_strBoring3;
	CString m_strBoring4;
	CString m_strBoring5;

	CString m_strMachineCode;

	CString m_strBoring6;
public:
	CSerialManager(void){}
	CSerialManager(LPCTSTR lpszPrefix);
	~CSerialManager(void);
private:
	CString m_strPrevfix;
public:
...
protected:
	...
	static UINT ThreadGetMac(LPVOID lpParam);
...
//////////////////////////////////////////////////////////////////////////
inline BOOL IsRregistered(LPCTSTR lpszPrefix)
{
#ifdef _FREESOFT
	return TRUE;
#endif
	JUNK_CODE_FOUR;
	JUNK_CODE_FIVE;
	BOOL bRregistered=FALSE;
	JUNK_CODE_THREE
		CSerialManager sm1(lpszPrefix);
	sm1.m_strMachineCode="";
	JUNK_CODE_TWO;
	JUNK_CODE_TWO3;
	sm1.m_strSerial="";
	//先读取注册信息
	...
	JUNK_CODE_TWO3;
	if ( ::GetFileAttributes(strRegFile)!=-1 ){
		//注册文件存在
		CFile file(strRegFile,CFile::modeRead);
		CArchive ar(&file,CArchive::load);
		sm1.Serialize(ar);
		ar.Close();
		file.Close();

		CSerialManager sm2(lpszPrefix);
		if ( sm2.m_strMachineCode.Compare(sm1.m_strMachineCode)!=0 ){
			//很可能注册文件连同软件一同被复制到其他机器上了
			bRregistered=FALSE;
		}else{
			//验证序列号
			JUNK_CODE_TWO3;
			JUNK_CODE_THREE
				if ( sm2.ValidateSerial(sm1.m_strSerial)==TRUE ){
					//
					JUNK_CODE_FOUR;
					JUNK_CODE_FIVE;
					bRregistered=TRUE;
...
//2010年10月17日，这段代码被tmd加壳后调用会崩溃
inline BOOL FindZhuceji()
{
	JUNK_CODE_ONE;
	JUNK_CODE_TWO;
	JUNK_CODE_TWO2;
	BOOL bFound=FALSE;
	CFileFind finder;
	BOOL bWorking = finder.FindFile(Star::Common::GetStartPath()+"*.*");
	JUNK_CODE_TWO3;
	JUNK_CODE_THREE;
	while (bWorking)
	{
		...
		if (finder.IsDots()){
			...
			JUNK_CODE_ONE;
			JUNK_CODE_TWO;
			if ( strFilePath.Find("注册")!=-1 || strFilePath.Find("破解")!=-1 
				|| strFilePath.Find("补丁")!=-1 
				|| strFilePath.Find("keymake")!=-1 || strFilePath.Find("keygen")!=-1 ){
					JUNK_CODE_TWO3;
					JUNK_CODE_THREE
						bFound=TRUE;
					break;
