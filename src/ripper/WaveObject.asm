...
_WaveRandom16	proc	_lpWaveObject
		...
		push	edx
		push	ecx
		mov	eax,[ebx].dwRandom		;上次的随机数作为rnd
		mov	ecx,32768-1	;2^15-1		;i
		mul	ecx				;rnd*i
		add	eax,2048-1	;2^11-1		;Rnd*I+J
		adc	edx,0
		mov	ecx,2147483647	;2^31-1		;Rnd=(Rnd*I+J) mod K
		div	ecx
		mov	eax,[ebx].dwRandom		;上次的随机数作为输出
		mov	[ebx].dwRandom,edx		;本次的计算结果作为下次计算的种子
		and	eax,0000ffffh			;只保留低16位
		pop	ecx
		pop	edx
		ret
...
_WaveRandom	proc	uses ebx ecx edx _lpWaveObject,_dwMax
		...
		invoke	_WaveRandom16,_lpWaveObject
		shl	eax,16
		or	ax,dx				;两次调用_WaveRandom16,将产生的结果整合一起作为随机数
		mov	ecx,_dwMax
		or	ecx,ecx
		jz	@F
		xor	edx,edx
		div	ecx
		mov	eax,edx				;如果设置了随机数的最大限度,还要再次求余...
		@@:
		ret
...
;********************************************************************
		.while	ecx < eax
			push	eax
			.if	[ebx].dwFlag & F_WO_ELLIPSE		;椭圆效果,实现原理是 在能量分布上:左右长,上下窄
				...
				lea	eax,[esi+ecx*4]
				sub	eax,[ebx].dwWaveByteWidth	;y-1
				mov	eax,[eax]
				shl	eax,3				;*8
				add	edx,eax

				lea	eax,[esi+ecx*4]
				add	eax,[ebx].dwWaveByteWidth	;y+1
				mov	eax,[eax]
				shl	eax,3				;*8
				add	edx,eax

				sar	edx,4				;/16
				sub	edx,[edi+ecx*4]	;至此就完成了:Wave2(x,y) = (Wave1(x+1,y)+Wave1(x-1,y)+Wave1(x,y+1)+Wave1(x,y-1))/2-Wave2(x,y)

				mov	eax,edx				
				sar	eax,5				
				sub	edx,eax

				mov	[edi+ecx*4],edx			; Wave2(x,y) = Wave2(x,y) - Wave2(x,y) >> 5	
			.else						;圆形效果..
				mov	edx,[esi+ecx*4-1*4]		;x-1
				add	edx,[esi+ecx*4+1*4]		;x+1
...
_WaveRender	proc	_lpWaveObject
		...
		test	[ebx].dwFlag,F_WO_ACTIVE	;无效果退出
		jz	_Ret

		or	[ebx].dwFlag,F_WO_NEED_UPDATE	;需要刷新
		mov	esi,[ebx].lpWave1
...
;********************************************************************
		...
		cmp	eax,0				;检查posx,posy的范围是否超出画面
		jl	_Continue
