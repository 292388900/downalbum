...
typedef struct tag_ExifInfo {
	char  Version      [5];
    char  CameraMake   [32];	//生产者 指产品生产厂家
    char  CameraModel  [40];	//型号 指设备型号
    char  DateTime     [20];	//日期和时间
    int   Height, Width;
    int   Orientation;
    int   IsColor;
    int   Process;
    ...
    float FocalLength;
    float ExposureTime;			//曝光时间 即快门速度
    float ApertureFNumber;		//光圈系数
    float Distance;
    float CCDWidth;
    float ExposureBias;			//曝光补偿。
    int   Whitebalance;			//光源 指白平衡设置
    int   MeteringMode;
    int   ExposureProgram;		//曝光程序 指程序式自动曝光的设置，各相机不同，可能是Shutter Priority（快门优先，Tv）、Aperture Priority（快门优先，Av）等等。
...
public:
	...
	//修改照片的拍摄时间
	bool UpdateOriginalDateTime(const char *lpszNewTime);
