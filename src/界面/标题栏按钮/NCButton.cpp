...
void CNCButton::OnPaint()
{
	...
	HBITMAP hMemBmp = CreateCompatibleBitmap(hdc, m_sizeBtn.cx, m_sizeBtn.cy);
	...
	{
		OnPaintNormal(hMemDC);
	}
	// 拷贝到屏幕

	BitBlt(hdc, m_Rect.left, m_Rect.top, m_sizeBtn.cx, m_sizeBtn.cy, hMemDC, 0, 0, SRCCOPY);
...
void CNCButton::OnPaintNormal(HDC hdc)
{
	...
	if(nBmpID != 0)
		DrawTransparentBmp(hdc, 2, 1, 10, 10, nBmpID, RGB(0xD8, 0xD4, 0xD0));
...
void CNCButton::OnPaintDown(HDC hdc)
{
	...
	if(nBmpID != 0)
		DrawTransparentBmp(hdc, 3, 2, 10, 10, nBmpID, RGB(0xD8, 0xD4, 0xD0));
